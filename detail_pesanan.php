...
<head>
    <meta charset="UTF-8">
    <title>Detail Pesanan #GW-<?php echo $id_pesanan; ?> - Wartan</title>
...
<body class="bg-gray-100 p-8">
    ...
    <div class="max-w-3xl mx-auto bg-white rounded-lg shadow-lg overflow-hidden">
        ...
        <div class="bg-wartan text-white p-4 flex justify-between items-center">
            <h2 class="text-xl font-bold">Detail Pesanan #GW-<?php echo $id_pesanan; ?></h2>
            <a href="<?php echo ($_SESSION['role'] == 'admin') ? 'pesanan.php' : 'riwayat.php'; ?>" class="bg-white text-green-700 px-3 py-1 rounded text-sm font-semibold hover:bg-gray-100 transition">Kembali</a>
        </div>
        ...
        <div class="p-6">
            <div class="grid grid-cols-2 gap-4 mb-6 bg-gray-50 p-4 rounded-lg border border-gray-200">
                <div>
                    ...
                    <p class="font-bold text-gray-800"><?php echo htmlspecialchars($data_pesanan['nama']); ?></p>
                </div>
                <div>
                    <p class="text-sm text-gray-500">Waktu Pemesanan</p>
                    <p class="font-bold text-gray-800"><?php echo date('d/m/Y H:i', strtotime($data_pesanan['tanggal_pesanan'])); ?></p>
                </div>
                <div>
                    <p class="text-sm text-gray-500">Tipe Pesanan</p>
                    <p class="font-bold text-gray-800 uppercase">ONLINE</p>
                </div>
                <div>
                    <p class="text-sm text-gray-500">Alamat Pengiriman</p>
                    <p class="font-bold text-gray-800"><?php echo htmlspecialchars($data_pesanan['alamat_pengiriman'] ? $data_pesanan['alamat_pengiriman'] : '- (Ambil di tempat)'); ?></p>
                </div>
            </div>
            ...
            <table class="w-full text-left mb-6 border-collapse">
                ...
                <tfoot>
                    <tr class="bg-gray-50 font-bold text-lg">
                        ...
                        <td class="p-3 text-right text-green-700">Rp <?php echo number_format($data_pesanan['total_pesanan'], 0, ',', '.'); ?></td>
                    </tr>
                </tfoot>
            </table>
            ...
            <div class="border-t pt-6 mt-6">
                <form action="actions/proses_status.php" method="POST" class="flex items-end gap-4">
                    ...
                    <div class="flex-1">
                        ...
                        <select name="status_pesanan" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-green-500">
                            <option value="pending" <?php if($data_pesanan['status_pesanan'] == 'pending') echo 'selected'; ?>>Pending (Menunggu)</option>
                            <option value="diproses" <?php if($data_pesanan['status_pesanan'] == 'diproses') echo 'selected'; ?>>Diproses (Sedang Dimasak)</option>
